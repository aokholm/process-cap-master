...
We have focused on $C_{pk}$ through most of this section, however using $C_{pm}$ would potentially be better if the desired tolerances directly influence product performance - for instance in lens optics construction. 

\subsection{Normalization}

The data is normalised to relate the specify tolerance limits independent of size. By normalising PC data it can be used to predict PC for any dimension within the limit of the normalisation algorithm. 
...
In a note for ANSI B4-2  and later mentioned in ISO 286:1993 a continues function is described for IT-grades between IT6 to IT16 for dimensions from 2 mm to 500 mm. This function is not included in newer versions of ISO 286.
...
\emph{Case 1. The user is interested in selecting a tolerance for a component of subset of specific DC's.}

Figure ~\ref{fig:acumfreq} shows the data from the selected subset in a normalised form. A normal distribution fitted to the data in an accumulated frequency plot. From the plot it is shown that  90 \% of the sample sets 
...
visible confidence intervals of interpretation
...
General
	...
	User can set tolerance (from IT to specific dimensions)
