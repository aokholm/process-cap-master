...
Higher mean shift from target $C_a$ will also slightly increase the certainty especially at low sample sizes.
...
The randomly generated ITG it is converted into a normal distribution of individual measurements X using a fixed target dimension of $m = 100 \mathrm{\ [mm]}$ and closeness to target $C_a = 0.9$. From this input the standard deviation $\sigma_{X}$ and mean shift $| \mu - m|$ can be calculated yielding the distribution parameters $\mathrm{X} \sim \mathcal{N} (\mu_{\mathrm{X}}, {| \mu - m|}^2)$. 
