...
Three parts were made of POM (PTFE filled) in a multi cavity mould. 
The last part were made using a blend of PC and a ABS.
...
One result is the overall distribution of process capability driven IT grades shown in figure \ref{fig:acumfreq}. 


The 90 \% cumulative frequency corresponds to an $14.2 \pm 0.4 $ IT grade. 
If the same production method and materials were going to used for a new project the 14.2 IT grade would be the recommended minimum tolerance. 
One very interesting related finding were the very small (or non existing)  correlation between the specified tolerance and the actual process capability driven tolerance as seen in figure \ref{fig:ITG_ITGSpec}. 
This clearly shows the need for process capability driven tolerances as a part of robust design. 
The dotted line represent the same 90 \% cumulative frequency as shown in figure \ref{fig:acumfreq}. 
The reason for choosing 90 \% cumulative frequency is related to a large degree of exceptions in the upper 10 \% of the measurements sets.
These exceptions include dimensions of: long and slender objects, unnecessary tight tolerances in non-critical areas and dimensions where the specifications has been come dissynchronised with the manufacturing targets. 
...
In the quest for finding design characteristics which results in a tighter permissible tolerances we have tried to analyse our sample data for multiple different design characteristics. This is the results:
\begin{itemize}

\item Linear dimension within a single mould half did not show significant better performance than dimensions across mold split lines. 

\item Small, medium and large dimensions performed to the same required tolerance. The dimensions were divided into bins of $[ 0<x \le 5 ]$, $[5<x\le 10 ] $, $[10<x] \ \mathrm{(mm)}$. 
Dimensions in the range $[5<x \le 10]$ seems to have a lower required tolerance grade, but more measurements were needed in order to have statistical significance.
...
With more data it would be very interesting to investigate 
\begin{itemize}
