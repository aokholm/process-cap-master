...
All manufacturing processes produces parts with variation of their specified dimensions.  
...
Early research focused on characterising the variation and understanding the related quality loss \cite{taguchi1986introduction}. 
...
\textbf{Fragmented Organisation.}  A PCDB is a cross department project  usually involving manufacturing, quality control and mechanical design within the company. 
...
The databases are typically not easily searchable which makes it very difficult to find the correct data and in many cases the data sought after does not exists. 
...
The first result is the overall distribution of process capability driven IT grades shown in figure \ref{fig:acumfreq}. 
The 90 \% cumulative frequency corresponds to an $14.2 \pm 0.4 $ IT grade. If the same production method and materials were going to used for a new project the 14.2 IT grade would be the recommended minimum tolerance. In the quest for finding design characteristics which would result in a tighter permissible tolerance we have tried to analyse our sample data for multiple different design characteristics. 

X

Comparison of process capabilities for different design characteristics are useful. 
In figure \ref{fig:Cb_holeshaft} are internal and external measurements compared for $C_b$ normalised bias.
The is a tendency, the internal measurements (holes) are bigger than the external measurements (shafts) which can be explained by: early production run, purposely under dimensioned in production to ensure assembly, etc. 

In the test case we tried comparing addtional design characteristics.
\begin{itemize}

\item Material specific process capability was compared. For the test database, the material data was confounded to multi cavity mold vs. single cavity and hard steel vs. soft steel mold. No meaningful conclusion.

\item Added variance from measurements across mold split lines. No significant difference. 

\item Dimension specific process capability. Ex the PC of $[ 0<x \le 5 ]$, $[5<x\le 10 ] $, $[10<x] \mathrm{mm}$ dimensions compared. Measurements in the range $[5<x \le 10]$ are more precise but too little data to make a general describtion.

\item Effects circular measurements compared to positional. The CT- scanner determines the diameter based on multiple points, which results in addition precision compared to positional measurements.
...
Traditionally mechanical designer apply tolerances needed for functionality of a product with little regard process capability. Unfortunately it seems like there is very little correlation between the specified tolerance and the actual process capability driven tolerance as seen in figure \ref{fig:ITG_ITGSpec}.
Robust design methodology propose method of design based on process capability. A design that at robust in relation to variance The dotted line in figure \ref{fig:ITG_ITGSpec} represent 90 \% of data corresponding to a IT grade of $14.2$ or 90 \% probability in figure \ref{fig:acumfreq}. A design that in robust in relation to variance 
The reason for not choosing the IT-grade corresponding to 100 \% is related to exceptions in the data that create unintionally big tolerances. In the test data is measurements of: long and slender object, unnecessary tight tolerances in non-critical areas and areas that has been changed but changes are shown in the 'Intended design files'. 
...
\caption{\label{fig:Cb_holeshaft} Comparison of two different design characteristics. 
...
In figure \ref{fig:Cb_holeshaft} are internal and external measurements compared for $C_b$ normalised bias.
The is a tendency, the internal measurements (holes) are bigger than the external measurements (shafts) which can be explained by: early production run, purposely under dimensioned in production to ensure assembly, etc. 
A test case has been measured and analyzed to verify assumption in the proposal and to provide a hands on example.
